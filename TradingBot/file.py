...
class TelegramScalpingBot:
    def __init__(self):
        ...
        print("🚀 ASTARBOT v3.1: Akıllı Zaman Stopu (Kârda Kapat) Aktif!")
    ...
    async def check_tracked_trades(self):
        ...
        for i, ticker in enumerate(tickers):
            ...
            # --- YENİ MANTIK: AKILLI ZAMAN STOPU ---
            elapsed = (datetime.now() - trade['time']).total_seconds() / 3600 # Saat cinsinden
            
            if elapsed >= Config.TIME_STOP_HOURS:
                # LONG ise ve fiyat girişin üstündeyse (Kârda)
                if trade['side'] == "LONG" and price > trade['entry']:
                    await self.send_telegram(f"⏰ <b>{symbol} Zaman Aşımı:</b> Kârda olduğu için kapatıldı. 💰")
                    del self.tracked_trades[symbol]
                    continue
                # SHORT ise ve fiyat girişin altındaysa (Kârda)
                elif trade['side'] == "SHORT" and price < trade['entry']:
                    await self.send_telegram(f"⏰ <b>{symbol} Zaman Aşımı:</b> Kârda olduğu için kapatıldı. 💰")
                    del self.tracked_trades[symbol]
                    continue
                # NOT: Zararda ise dokunmuyoruz, teknik stop (SL) bekliyor.
    ...
    async def run_forever(self):
        ...
        await self.send_telegram("🚀 <b>ASTARBOT v3.1 AKTİF</b>\nZaman Stopu: Kârda Kapat, Zararda Bekle.")
        asyncio.create_task(self.monitor_loop())
        while True:
            now = datetime.now()
            wait_sec = ((Config.SCAN_INTERVAL_MINUTES - (now.minute % Config.SCAN_INTERVAL_MINUTES)) * 60) - now.second
            if 5 < wait_sec < (Config.SCAN_INTERVAL_MINUTES * 60 - 5): await asyncio.sleep(wait_sec)
...
if __name__ == "__main__":
    asyncio.run(TelegramScalpingBot().run_forever())
